...
<div class="w3-main" style="margin-left:300px;margin-top:43px;">
  ...
  <header class="w3-container" style="padding-top:22px">
    <h5><b><i class="fa fa-dashboard"></i> Inventory Sales & Profit Chart </b></h5>
  </header>

  <!-- content body -->
  <div class="w3-container">
    
    <form method="post" action="total_sales_inv.jsp">
    <div class="form-row">
    <div class="form-group">
    
    <label for="inventory">Select an inventory item:</label>
  ...
  if (request.getMethod().equals("POST")) {
    ...
    out.println("<center><h2>" + invName + " Total Sales and Profits in " + year + "</h2></center>");
  }
%>
  ...
  <% if (request.getMethod().equals("POST")) {
        ...
        int[] profits = new int[12]; // Array to store total profits for each month

        // Query the order table to get the total inventory sold and total profits for each month
        rs = stmt.executeQuery("SELECT SUM(i.totalSoldQty * i.sellingPrice) as total_sales, SUM(i.totalSoldQty * (i.sellingPrice - i.purchasePrice)) as total_profits, MONTH(o.orderDate) as month FROM ordersori o JOIN inventory i ON o.invId = i.invId WHERE YEAR(o.orderDate) = " + year + " AND i.invId = " + invId + " GROUP BY MONTH(o.orderDate)");
       
        while (rs.next()) {
          sales[rs.getInt("month") - 1] = rs.getInt("total_sales"); // Store the total sales for the month in the array
          profits[rs.getInt("month") - 1] = rs.getInt("total_profits"); // Store the total profits for the month in the array
        }

        // Display the chart using Chart.js
        out.println("<canvas id=\"salesChart\" height=\"300px\" width=\"600px\"></canvas>");
        out.println("<script>");
        out.println("var salesData = {");
        out.println("  labels: [\"Jan\", \"Feb\", \"Mar\", \"Apr\", \"May\", \"Jun\", \"Jul\", \"Aug\", \"Sep\", \"Oct\", \"Nov\", \"Dec\"],");
        out.println("  datasets: [{");
        out.println("    label: \"Total Sales\",");
        out.println("    data: [" + sales[0] + ", " + sales[1] + ", " + sales[2] + ", " + sales[3] + ", " + sales[4] + ", " + sales[5] + ", " + sales[6] + 
       ", " + sales[7] + ", " + sales[8] + ", " + sales[9] + ", " + sales[10] + ", " + sales[11] + "],");

        out.println(" backgroundColor: \"#B956B4\",");
        out.println(" borderColor: \"#B956B4\",");
        out.println(" borderWidth: 1");
        out.println(" }, {");
        out.println(" label: \"Total Profits\",");
        out.println(" data: [" + profits[0] + ", " + profits[1] + ", " + profits[2] + ", " + profits[3] + ", " + profits[4] + ", " + profits[5] + ", " + profits[6] +
        ", " + profits[7] + ", " + profits[8] + ", " + profits[9] + ", " + profits[10] + ", " + profits[11] + "],");
        out.println(" backgroundColor: \"#F7CA69\",");
        out.println(" borderColor: \"#F7CA69\",");
        out.println(" borderWidth: 1");
        out.println(" }]");
        out.println("};");

		    out.println("var ctx = document.getElementById(\"salesChart\");");
